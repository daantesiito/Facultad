	...
	public List <List<String>> recorridosMasSeguro() {
		...
		if (comienzo != null) {
			...
			if (destino != null) {
				...
				dfs(comienzo, destino, recorridoActual, recorridos, marca);
			}
		}
		return recorridos;
	}
	
	public void dfs(Vertex<String> vert, Vertex<String> destino, LinkedList<String> recorridoActual, LinkedList<LinkedList<String>> recorridos, boolean[] marca) {
		...
		if (vert.equals(destino)) {
			...
			for (Edge<String> a: ady) {
				if (a.getWeight() < 5 && !marca[a.getTarget().getPosition()]) {
					dfs(a.getTarget(), destino, recorridoActual, recorridos, marca);
				}
			}
		}
